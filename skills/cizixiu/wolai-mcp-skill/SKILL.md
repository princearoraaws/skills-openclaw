...
5. 点击右上角**创建Token**按钮，创建后复制Token令牌字符串
...
首次使用技能时，AI 会检查是否已配置 MCP Token。

**如果没有配置**，AI 会提示：
> 「未检测到 wolai MCP Token。请打开 wolai → 个人设置 → MCP 接入 创建 Token，然后告诉我 Token，我来帮你配置。」

**配置完成后**，后续使用就无需再配置了（环境变量会保留）。
...
if (-not $env:WOLAI_MCP_TOKEN) {
    ...
    Write-Host ""
    Write-Host "请先获取 Token："
    Write-Host "1. 打开 https://www.wolai.com 登录"
    Write-Host "2. 点击左下角 → 个人设置 → MCP 接入"
    Write-Host "3. 点击「创建Token」并复制"
    Write-Host ""
    Write-Host "然后告诉我：「帮我配置 wolai MCP Token，Token 是 sk-xxxxxx」"
    exit 1
}

Write-Host "✅ Token 已配置: $($env:WOLAI_MCP_TOKEN.Substring(0, 8))..."
```
...
# 方式一：直接设置环境变量
os.environ["WOLAI_MCP_TOKEN"] = "sk-xxxxxx"

# 方式二：运行时传入（如果有自定义获取方式）
# token = get_token_from_user_input()
# os.environ["WOLAI_MCP_TOKEN"] = token
```
