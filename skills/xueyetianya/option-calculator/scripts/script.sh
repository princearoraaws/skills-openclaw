#!/usr/bin/env bash
# option-calculator - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${OPTION_CALCULATOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/option-calculator}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
option-calculator v$VERSION
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "option-calculator v$VERSION" ;;
