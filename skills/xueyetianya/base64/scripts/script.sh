...
DATA_DIR="${HOME}/.local/share/base64"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "base64 v2.0.0"; }

_help() {
    echo "Base64 v2.0.0 — utility toolkit"
    echo ""
    echo "Usage: base64 <command> [args]"
    echo ""
    echo "Commands:"
    echo "  run                Run"
    echo "  check              Check"
...
_search() {
    local term="${1:?Usage: base64 search <term>}"
...
case "${1:-help}" in
    run)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/run.log" 2>/dev/null || echo "  No entries yet. Use: base64 run <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/run.log"
            local total=$(wc -l < "$DATA_DIR/run.log")
            echo "  [Base64] run: $input"
    ...
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: base64 check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [Base64] check: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: base64 convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [Base64] convert: $input"
    ...
    analyze)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/analyze.log" 2>/dev/null || echo "  No entries yet. Use: base64 analyze <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/analyze.log"
            local total=$(wc -l < "$DATA_DIR/analyze.log")
            echo "  [Base64] analyze: $input"
    ...
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: base64 generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [Base64] generate: $input"
    ...
    preview)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/preview.log" 2>/dev/null || echo "  No entries yet. Use: base64 preview <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/preview.log"
            local total=$(wc -l < "$DATA_DIR/preview.log")
            echo "  [Base64] preview: $input"
    ...
    batch)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/batch.log" 2>/dev/null || echo "  No entries yet. Use: base64 batch <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/batch.log"
            local total=$(wc -l < "$DATA_DIR/batch.log")
            echo "  [Base64] batch: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: base64 compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Base64] compare: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: base64 export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Base64] export: $input"
    ...
    config)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/config.log" 2>/dev/null || echo "  No entries yet. Use: base64 config <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/config.log"
            local total=$(wc -l < "$DATA_DIR/config.log")
            echo "  [Base64] config: $input"
    ...
    status)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/status.log" 2>/dev/null || echo "  No entries yet. Use: base64 status <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/status.log"
            local total=$(wc -l < "$DATA_DIR/status.log")
            echo "  [Base64] status: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: base64 report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Base64] report: $input"
    ...
    *)
        echo "Unknown: $1 — run 'base64 help'"
        exit 1
        ;;
esac