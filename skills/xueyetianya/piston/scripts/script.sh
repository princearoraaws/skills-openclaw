#!/usr/bin/env bash
# piston -- Piston and cylinder design calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PISTON_DIR:-$HOME/.piston}"
...
show_help() {
    cat << EOF
piston v$VERSION -- Piston and cylinder design calculator

Usage: piston <command> [args]
...
cmd_add() {
    local value="${1:?Usage: piston add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: piston search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: piston remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "piston v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: piston help"; exit 1 ;;
esac
