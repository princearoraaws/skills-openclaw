...
description: >
  题爪 · 国考备考追踪 Skill。当用户发来套题成绩、错题截图、备考打卡或复习进度时触发。
...
本 Skill 会执行以下操作，均在本地完成：

- **读取环境变量**：从 OpenClaw 的 auth-profiles 读取多模态模型 API Key，仅用于识别你发来的截图
- **对外网络请求**：仅向你配置的模型 API 发送图片（如 dashscope.aliyuncs.com、api.moonshot.cn），不向其他地址发送任何数据
- **执行 Python 子进程**：调用 openpyxl 生成 Excel 文件，不访问网络
- **写入本地文件**：所有错题数据写入 `~/.openclaw/skills/kaogong-study-tracker/data/`，不上传任何云端

No data leaves your machine except the API calls to your configured multimodal model.

# 题爪 · 国考备考追踪 Skill

## 一、首次安装（onboarding）

Skill 首次加载时（`config.json` 不存在或 `setup_done` 不为 true），主动发一条消息，
在聊天里完成配置，用户不需要手动编辑任何文件。
...
发截图前先告诉我你用什么多模态模型？直接输入名字就行，
比如 qwen3-vl、kimi-k2.5、claude-sonnet-4-6……
...
写入 `config.json`（**只有这两个字段**）：

```json
{ "setup_done": true, "model": "qwen3-vl" }
```
...
```
好，记住了，用「qwen3-vl」识别截图。

之后直接发错题截图就行。
如果哪次识别失败，把题目文字复制过来发给我，一样能整理。
```
...
题爪不收集 API Key，Key 在 OpenClaw 的 `auth-profiles.json` 里统一管理。
题爪根据模型名推断 provider，自动读取对应的环境变量（如 `DASHSCOPE_API_KEY`、`MOONSHOT_API_KEY` 等）。
...
| "只导出待二刷的"                           | → export_xlsx --pending-only |
