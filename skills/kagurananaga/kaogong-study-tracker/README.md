...
题爪想解决的就是这件事：**让"做完题之后的整理"变得几乎不需要努力**。你只需要把错题截图发出来，或者发一句"今天判断推理错了8道"，剩下的它来做。
...
发一张错题截图，题爪调用你配置的多模态模型（qwen3-vl、kimi-k2.5、claude-sonnet-4-6 等），自动提取：
...
OpenClaw 支持哪个渠道，题爪就支持哪个，逻辑层完全平台无关。
...
- 一个支持图片输入的多模态模型 API Key（见下方说明）

### 多模态模型选择

题爪需要一个能"看图"的 AI 来识别截图。安装后会引导你配置，常见选项：

| 模型 | 提供方 | 国内访问 | 获取地址 |
|------|--------|---------|---------|
| qwen3-vl | 阿里通义 | 直连 | dashscope.aliyuncs.com |
| kimi-k2.5 | Moonshot | 直连 | platform.moonshot.cn |
| glm-4v | 智谱 | 直连 | open.bigmodel.cn |
| minimax-m2.5 | MiniMax | 直连 | api.minimax.chat |
| claude-sonnet-4-6 | Anthropic | 需代理 | console.anthropic.com |
| gpt-5.2 | OpenAI | 需代理 | platform.openai.com |
| gemini-3.1-pro | Google | 需代理 | aistudio.google.com |

API Key 在 OpenClaw 的 `auth-profiles.json` 里统一管理，题爪根据模型名自动推断调用方式，不需要额外配置。
...
cd ~/.openclaw/skills
git clone https://github.com/你的用户名/kaogong-study-tracker
...
安装后发任意消息，题爪会主动问你用什么多模态模型，输入模型名即可完成配置，全程在聊天里完成，不需要编辑任何文件。
