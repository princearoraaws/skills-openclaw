...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if "accounts" not in data or "default_account" not in data:
        raise RuntimeError("配置文件缺少 accounts 或 default_account 字段")
    if not data["accounts"]:
        raise RuntimeError("配置中 accounts 字典为空")
    return data


def get_account_config(cfg: dict[str, Any], account_name: str | None) -> dict[str, Any]:
    """从配置中获取指定账号的配置信息。"""
    accounts = cfg["accounts"]
    if not account_name:
        account_name = cfg.get("default_account") or list(accounts.keys())[0]
    if account_name not in accounts:
        available = ", ".join(accounts.keys())
        raise RuntimeError(f"账号 '{account_name}' 不在 accounts 列表中。可用账号: {available}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--account", default="", help="指定要使用的公众号账号（账号名），默认使用 default_account")
    parser.add_argument("--list-accounts", action="store_true", help="列出配置中的所有公众号账号")
...
def main() -> None:
    ...
    account_display = wechat_cfg.get("name", "default")
