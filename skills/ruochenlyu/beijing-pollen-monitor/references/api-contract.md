...
```bash
./scripts/beijing-pollen-query.sh query --mode <report|overview|stations|history> [options]
```
...
| 参数 | 说明 | 默认值 |
|---|---|---|
| `--mode` | `report`、`overview`、`stations`、`history` | `overview` |
| `--district` | 区名，模糊匹配（report/history 必填） | — |
...
```json
{
  ...
  "query": { "mode": "report", "district": "海淀", "format": "json", "limit": 5 },
  "data": {},
  "warnings": []
}
```
...
```json
{
  ...
  "generated_at": "...",
  "query": {},
  "data": { "text": "北京花粉日报（2026-03-30 08:00）\n\n关注站点：海淀区\n..." },
  "warnings": []
}
```
...
`data` 字段：
- `beijing_areas[]` — area_id, name, raw_pollen, display_value, level_code, level_text, observed_at
- `stations[]` — station_id, station_name, raw_pollen, display_value, level_code, level_text, observed_at, advice
...
- `series[]` — timestamp, raw_pollen, display_value, numeric_value, level_code, level_text, advice
- `summary` — latest_value, latest_level.code/text, latest_advice, min, max, delta_24h, trend
