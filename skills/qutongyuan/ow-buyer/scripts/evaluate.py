...
def evaluate_bids(req_id: str) -> Dict[str, Any]:
    """执行完整评标"""
    ...
    for bid in bids:
        ...
        results.append({
            ...
            "auth_docs": [d["name"] for d in bid.get("auth_docs", [])],
...
def format_result(result: Dict) -> str:
    ...
    for i, bid in enumerate(result["top3"]):
        ...
        output += f"   ├─ 时间：{bid['delivery_days']}天 (得分 {bid['delivery_score']:.1f}/10)\n"
        output += f"   └─ 信誉：{bid['reputation'].get('total_transactions', 0)}次成交 (得分 {bid['reputation_score']:.1f}/20)\n"
        output += "\n"
