#!/usr/bin/env bash
# observability — Observability reference tool. Use when working with observability in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
observability v$VERSION — Observability Reference Tool

Usage: observability <command>
...
Observability (observability) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with observability.

## Key Concepts
- Core observability principles and fundamentals
- How observability fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Observability Matters
Understanding observability is critical for:
...
## Getting Started
1. Understand the basic observability concepts
...
## Installation
1. Download or clone the observability package
...
cmd_patterns() {
    cat << 'EOF'
# Observability — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for observability
...
cmd_migration() {
    cat << 'EOF'
# Observability — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "observability v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: observability help"; exit 1 ;;
esac
