#!/usr/bin/env bash
# unbonding -- Analyze unbonding operations. Use when you need to understand unbonding mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${UNBONDING_DIR:-$HOME/.unbonding}"
...
show_help() {
    cat << EOF
unbonding v$VERSION -- Analyze unbonding operations. Use when you need to understand unbonding mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: unbonding <command> [args]
...
cmd_add() {
    local value="${1:?Usage: unbonding add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: unbonding search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: unbonding remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="unbonding-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "unbonding v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: unbonding help"; exit 1 ;;
esac
