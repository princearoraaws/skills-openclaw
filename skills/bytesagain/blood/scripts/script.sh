...
DATA_DIR="${HOME}/.local/share/blood"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "blood v2.0.0"; }

_help() {
    echo "Blood v2.0.0 — health toolkit"
    echo ""
    echo "Usage: blood <command> [args]"
    echo ""
    echo "Commands:"
    echo "  log                Log"
    echo "  track              Track"
...
case "${1:-help}" in
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: blood log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/log.log"
            local total=$(wc -l < "$DATA_DIR/log.log")
            echo "  [Blood] log: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: blood track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Blood] track: $input"
    ...
    chart)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/chart.log" 2>/dev/null || echo "  No entries yet. Use: blood chart <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/chart.log"
            local total=$(wc -l < "$DATA_DIR/chart.log")
            echo "  [Blood] chart: $input"
    ...
    goal)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/goal.log" 2>/dev/null || echo "  No entries yet. Use: blood goal <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/goal.log"
            local total=$(wc -l < "$DATA_DIR/goal.log")
            echo "  [Blood] goal: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: blood remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Blood] remind: $input"
    ...
    weekly)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weekly.log" 2>/dev/null || echo "  No entries yet. Use: blood weekly <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weekly.log"
            local total=$(wc -l < "$DATA_DIR/weekly.log")
            echo "  [Blood] weekly: $input"
    ...
    monthly)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/monthly.log" 2>/dev/null || echo "  No entries yet. Use: blood monthly <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/monthly.log"
            local total=$(wc -l < "$DATA_DIR/monthly.log")
            echo "  [Blood] monthly: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: blood compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Blood] compare: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: blood export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Blood] export: $input"
    ...
    streak)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/streak.log" 2>/dev/null || echo "  No entries yet. Use: blood streak <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/streak.log"
            local total=$(wc -l < "$DATA_DIR/streak.log")
            echo "  [Blood] streak: $input"
    ...
    milestone)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/milestone.log" 2>/dev/null || echo "  No entries yet. Use: blood milestone <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/milestone.log"
            local total=$(wc -l < "$DATA_DIR/milestone.log")
            echo "  [Blood] milestone: $input"
    ...
    trend)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/trend.log" 2>/dev/null || echo "  No entries yet. Use: blood trend <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/trend.log"
            local total=$(wc -l < "$DATA_DIR/trend.log")
            echo "  [Blood] trend: $input"
