#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Article metadata sync, article detail fetching and local exports."""
...
def _parse_article_html(html: str, fallback_article: dict[str, Any] | None = None) -> dict[str, Any]:
    if "环境异常" in html or "当前环境异常" in html:
        raise WechatArticleAccessError("微信返回环境异常校验页，请配置代理后重试")

    soup = BeautifulSoup(html, "html.parser")
    container = soup.select_one("#js_content") or soup.select_one("#js_article")
    if not container:
        raise WechatArticleAccessError("页面中未找到文章正文容器")
...
def _download_images(
    ...
    LOGGER.debug("download_images article_id=%s enabled=%s", article_id, download_images)
    ...
    for index, node in enumerate(soup.find_all("img"), start=1):
        ...
        extension = _guess_extension(source)
        ...
        image_records.append(
            {
                "url": source,
                "local_path": str(local_path) if saved else "",
                "saved": saved,
                "error": error,
            }
        )
...
def _build_article_detail_payload(
    *,
    article_id: str,
    fakeid: str,
    aid_value: str,
...
def article_detail(
    ...
    LOGGER.info(
        "article_detail start aid=%s link=%s download_images=%s include_html=%s force_refresh=%s save_files=%s",
        aid,
        link,
        download_images,
        include_html,
    ...
    if not link:
        return failure("未找到文章链接，请提供 link 或确保 aid 已在本地数据库中存在")
    ...
    if cached and not force_refresh:
        LOGGER.info("article_detail cache_hit article_id=%s", article_id)
        payload = _build_article_detail_payload(
            article_id=article_id,
            fakeid=fakeid,
            aid_value=aid_value,
            link=link,
        ...
        return success(payload, f"已返回缓存文章详情: {article_id}")

    client = WechatMPClient(db)
    html = client.fetch_public_article(link)
    parsed = _parse_article_html(html, fallback_article=article)
    localized_html, image_records = _download_images(client, article_id, link, parsed["html_content"], download_images)
    ...
    base_name = safe_filename(f"{aid_value}_{parsed['title']}", fallback=stable_hash(link))
    ...
    LOGGER.info(
        "article_detail fetched article_id=%s images=%s saved_json=%s saved_md=%s include_html=%s",
        article_id,
        len(image_records),
        bool(saved_json_path),
        bool(saved_md_path),
        include_html,
    )
    ...
    with db.transaction():
        ...
        db.connection.execute(
            ...
            (
                ...
                link,
                parsed["digest"],
                str(article.get("cover") or "") if article else "",
                parsed["author_name"],
                create_time,
                now,
    ...
    return success(payload, f"已抓取文章详情: {parsed['title'] or article_id}")
