...
# Use an English commit message, for example:
# feat(task-id): TASK-ID: implement English task summary
# If the injected command below uses a non-English commit message, rewrite it in English before running it.

# 2. Notify the orchestrator (replace <> with actual values)
