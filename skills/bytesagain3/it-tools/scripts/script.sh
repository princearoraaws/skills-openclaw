#!/usr/bin/env bash
# It Tools — utility tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/it-tools"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "it-tools v2.0.0"; }

_help() {
    echo "It Tools v2.0.0 — utility toolkit"
    echo ""
    echo "Usage: it-tools <command> [args]"
    echo ""
    echo "Commands:"
    echo "  run                Run"
    echo "  check              Check"
...
_search() {
    local term="${1:?Usage: it-tools search <term>}"
...
case "${1:-help}" in
    run)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/run.log" 2>/dev/null || echo "  No entries yet. Use: it-tools run <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/run.log"
            local total=$(wc -l < "$DATA_DIR/run.log")
            echo "  [It Tools] run: $input"
    ...
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: it-tools check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [It Tools] check: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: it-tools convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [It Tools] convert: $input"
    ...
    analyze)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/analyze.log" 2>/dev/null || echo "  No entries yet. Use: it-tools analyze <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/analyze.log"
            local total=$(wc -l < "$DATA_DIR/analyze.log")
            echo "  [It Tools] analyze: $input"
    ...
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: it-tools generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [It Tools] generate: $input"
    ...
    preview)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/preview.log" 2>/dev/null || echo "  No entries yet. Use: it-tools preview <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/preview.log"
            local total=$(wc -l < "$DATA_DIR/preview.log")
            echo "  [It Tools] preview: $input"
    ...
    batch)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/batch.log" 2>/dev/null || echo "  No entries yet. Use: it-tools batch <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/batch.log"
            local total=$(wc -l < "$DATA_DIR/batch.log")
            echo "  [It Tools] batch: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: it-tools compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [It Tools] compare: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: it-tools export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [It Tools] export: $input"
    ...
    config)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/config.log" 2>/dev/null || echo "  No entries yet. Use: it-tools config <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/config.log"
            local total=$(wc -l < "$DATA_DIR/config.log")
            echo "  [It Tools] config: $input"
    ...
    status)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/status.log" 2>/dev/null || echo "  No entries yet. Use: it-tools status <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/status.log"
            local total=$(wc -l < "$DATA_DIR/status.log")
            echo "  [It Tools] status: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: it-tools report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [It Tools] report: $input"
    ...
    *)
        echo "Unknown: $1 — run 'it-tools help'"
        exit 1
        ;;
esac