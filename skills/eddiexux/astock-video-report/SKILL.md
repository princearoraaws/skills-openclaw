...
- 如果是 `CLOSED` / `NOT_OPEN`：数据为最近交易日。通过当前星期几推算：
  - 周一~周五 15:00 后 → 交易日 = 今天
  - 周一~周五 00:00~09:30 → 交易日 = 昨天（如果昨天是周末则往前推到周五）
  - 周六 → 交易日 = 周五
  - 周日 → 交易日 = 周五
- 如果是 `TRADING` / `CALL_AUCTION` / 其他 → 交易日 = 今天
- **将确定的交易日期用于后续所有 `--date` 参数**
